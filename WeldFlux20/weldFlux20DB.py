...
class WeldFlux20DB(AFXDataDialog):
    ...
    def __init__(self, form):
        ...
        pickHandler = WeldFlux20DBPickHandler(form, form.point1Kw1, form.point1Kw2,'Pick the start point', NODES|DATUM_POINTS, ONE,3, label)
        ...
        pickHandler = WeldFlux20DBPickHandler(form, form.point2Kw1,form.point2Kw2, 'Pick a point on weld line', NODES|DATUM_POINTS, ONE, 3,label)
        icon = afxGetIcon('select', AFX_ICON_SMALL )
        FXButton(p=pickHf, text='\tPick Items in Viewport', ic=icon, tgt=pickHandler, sel=AFXMode.ID_ACTIVATE,
            opts=BUTTON_NORMAL|LAYOUT_CENTER_Y, x=0, y=0, w=0, h=0, pl=2, pr=2, pt=1, pb=1)
        pickHf = FXHorizontalFrame(p=VAligner_4, opts=0, x=0, y=0, w=0, h=0,
        ...
        pickHandler = WeldFlux20DBPickHandler(form, form.point3Kw1,form.point3Kw2, 'Pick anothe point on weld line', NODES|DATUM_POINTS, ONE,4, self.p2Label)
        icon = afxGetIcon('select', AFX_ICON_SMALL )
        ...
        pickHandler = WeldFlux20DBPickHandler(form, form.point4Kw1,form.point4Kw2, 'Pick a point on weld toe', NODES|DATUM_POINTS, ONE, 2,label)
        icon = afxGetIcon('select', AFX_ICON_SMALL )
        FXButton(p=pickHf, text='\tPick Items in Viewport', ic=icon, tgt=pickHandler, sel=AFXMode.ID_ACTIVATE,
            opts=BUTTON_NORMAL|LAYOUT_CENTER_Y, x=0, y=0, w=0, h=0, pl=2, pr=2, pt=1, pb=1)
        ...
        HFrame_4 = FXHorizontalFrame(p=HFrame_3, opts=0, x=0, y=0, w=0, h=0,
            pl=0, pr=0, pt=0, pb=0)
        l = FXLabel(p=HFrame_4, text='            Select elements by:', opts=JUSTIFY_LEFT)
        FXRadioButton(p=HFrame_4, text='Box', tgt=form.EltypeKw1, sel=471)
        FXRadioButton(p=HFrame_4, text='Label', tgt=form.EltypeKw1, sel=472)

        HFrame_5 = FXHorizontalFrame(p=TabItem_4, opts=0, x=0, y=0, w=0, h=0,
            pl=0, pr=0, pt=0, pb=0)
        VFrame_1 = FXVerticalFrame(p=HFrame_5, opts=0, x=0, y=0, w=0, h=0,
            pl=0, pr=0, pt=0, pb=0) 
        ...
        self.Space=AFXTextField(p=VAligner_4, ncols=10, labelText='LayerSpace(mm):', tgt=form.SpaceKw, sel=0)
        ...
        self.addTransition(form.EnAMKw,AFXTransition.EQ,\
                True,self.PreName,\
                MKUINT(FXWindow.ID_ENABLE, SEL_COMMAND), None)
        self.addTransition(form.EnAMKw,AFXTransition.EQ,\
                True,self.FirstName,\
...
class WeldFlux20DBPickHandler(AFXProcedure):
        ...
        def __init__(self, form, keyword1,keyword2, prompt, entitiesToPick, numberToPick, highlightLevel, label):
                ...
                WeldFlux20DBPickHandler.count += 1
                self.setModeName('WeldFlux20DBPickHandler%d' % (WeldFlux20DBPickHandler.count) )
